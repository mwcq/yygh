<template>
    <div class="register">
        <div class="top">
            ...
            <div class="level">
                <svg t="1688486082246" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg"
                    p-id="7600" width="16" height="16">
                    <path
        ...
        <div class="deparment">
            <div class="leftNav"><ul>
                <li @click="changeIndex(index)" :class="{active:index==currentIndex}" v-for="(item,index) in hospitalStore.departmentArr" :key="item.depcode">{{ item.depname }}</li>
            </ul></div>
            <div class="deparmentinfo" >
                <!-- 用一个div代表大科室和小科室 -->
                <div class="showDeparment" v-for="(item) in hospitalStore.departmentArr" :key="item.depcode">
                    <h1 class="cur">{{ item.depname }}</h1>
                    <!-- 小科室 -->
                    <ul>
                        <li @click="showLogin" v-for="(item2) in item.children" :key="item2.depcode">{{ item2.depname }}</li>
                    </ul>
                </div>
            </div>
        </div>
...
.register {
    ...
    .content {
        ...
        .right {
            ...
            margin-left: 20px;
            font-size: 14px;
            .time ,.address,.route,.releasetim,.ruleInfo{
